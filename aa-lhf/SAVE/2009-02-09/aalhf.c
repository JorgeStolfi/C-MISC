    ...
    { real dhi, dlo;
      ...
      *gamma = (dhi + dlo)/2.0;
      ...
      { real ehi = shi - (*alpha) * xhi - *gamma;
        ...
        assert (abs(ehi) <= 1.0001 * (*delta), "cheb_inv bug");
        assert (abs(elo) <= 1.0001 * (*delta), "cheb_inv bug");
      }
    }
}
  ...
  if (xhi == xlo) 
    ...
    { real slo, shi, smd, xmd;
      ...
      *delta = (smd - slo - (*alpha) * (xmd - xlo))/2;
      ...
      { real ehi = shi - (*alpha) * xhi - *gamma;
        real elo = slo - (*alpha) * xlo - *gamma;
        assert (abs(ehi) <= 1.0001 * (*delta), "cheb_sqrt bug");
        assert (abs(elo) <= 1.0001 * (*delta), "cheb_sqrt bug");
      }
    }
}
        ...
        { /* Error extrema are -xmd(-), xmd(+) */
          ...
          *delta = tmd - (*alpha) * xmd;
          ...
          { real ehi = thi - (*alpha) * xhi;
            ...
            assert (abs(ehi) <= 1.0001 * (*delta), "cheb_tanh bug");
            assert (abs(elo) <= 1.0001 * (*delta), "cheb_tanh bug");
          }
        }
    }
}
