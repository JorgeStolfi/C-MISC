...
/* Last edited on 2023-01-14 12:27:08 by stolfi */
      ...
      for (y=0; y<NC; y++)
        for (z=0; z<NC; z++)
          { real xmin, xmax, ymin, ymax, zmin, zmax, wmin, wmax, fmin, fmax;
            ...
            /* if (bug) { printf("** error **\n"); nbug++; } */
            acc = log(wwid/(fwid + 1.0e-38))/log(2.0);
            bad = acc > accbad;
            if (bad) { accbad = acc; }
            if (bug | bad | nice_number(ntests))
              { printf("%3d %3d %3d", x, y, z);
                printf(" [%12.6f __ %12.6f] = %12.6f", wmin, wmax, wwid);
                printf(" [%12.6f __ %12.6f] = %12.6f", fmin, fmax, fwid);
                printf(" acc = %8.3f\n", acc);
              }
