/*
   ...
   Last edited on 2004-09-07 22:25:59 by stolfi
   Based on vtt 1.7 1998/05/09 RZ
   Changes:
   * Allowed extra "."s in locus name.
   * Allowed "#" inside {}-comments.
   * Added "-v" (print program version) option.
   * Added return values to "main" procedure.
   * Safer allocation of "folname" and "locname".
   * Added check for line buffer overflow.
   * Changed "if(x = y)" to "if((x = y) != 0)" to avoid warnings.
   Should check for overflow in "folname" and "locname", too.
*/
...
/* Maximum line length (including final '\0'): */
#define MAXLEN 512
...
char folname[8]     /* Folio name, e.g. "f85r3" */;
char locname[8]     /* Locus name, e.g. "C3", "L3.8", or blank */;
char lineauth= ' '  /* Name of transcriber or blank */;
char pgvar[27]      /* Page variable settings; pgvar[0] not used. */;
...
/* Decide whether to select page based on comparing page
   ...
   given by user. Page variable "@" abused to select 
   loci, making this a line selection option instead.
   The value of pgvar[0] is set in PrepLine. If iopt=0 check
...
char *argv[];
{
  ...
  /* Why not indeed: */
  char *what = "@(#)vtt\t1.7e1\t2004/09/07\tJS\n";
...
char *buf;
{
  ...
  while (cr == 0 && eod == 0) {
    ...
    if (cget == '#' && index == 0) {
      /* Hash symbols in invalid positions should be caught by PrepLine. */
      /* That includes "#" in column 1 of continuation lines. */
      hash = 1;
    }
...
char *buf1, *buf2;
{
  ...
  while ((cget = buf1[ind1]) != '\0') {
    ...
    if (hash) {
      ...
      /* 1. Check for unprotected hash later in line and change to = */
      if (cget == '#' && incomm == 0) {
        cwarn = cget;
        cget = '=';
      }
  ...
  if (hash == 0) {
    ...
    if (spec0 > 0) {
      fprintf(stderr, "%s", "Missing ; after &\n");
      return 1;
    }
    
    /* Set pgvar[0] if a new locus was read */
    if (hasfoli != 0 && newfoli == 0) {
      pgvar[0] = locname[0];
...
char *buf;
{
  int index=0, indout=0, output, ii;
  char cb;
  char wrapbuf[MAXLEN];
...
char *argv[];
{
  ...
  while (ilin == 0) {
    ...
    if ((selloc = selpage) != 0) selloc = usepage(0);
