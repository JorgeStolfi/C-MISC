...
* 28 Dec 93
*/
/* Last edited on 2023-01-14 12:21:11 by stolfi */
...
{
 XFillRectangle(display,window,gc,xmin,ymin,xmax-xmin+1,ymax-ymin+1);
}
...
void dvtext(int x, int y, char* s, char* m)
{
 ...
 if (opaque)
  XDrawImageString(display,window,gc,x,y,s,n);
 else
 ...
 for (m=marktype; *m!=0; m++)
  switch (*m)
  {
   ...
   case '.':
    ...
    break;
   case 'o':
    XDrawArc(display,window,gc,x-size,y-size,2*size,2*size,0,360*64);
    break;
   case 'O':
    XFillArc(display,window,gc,x-size,y-size,2*size,2*size,0,360*64);
    break;
   case '+':
    s[0].x1=x-size;	s[0].y1=y;	s[0].x2=x+size;	s[0].y2=y;
    s[1].x1=x;		s[1].y1=y-size;	s[1].x2=x;	s[1].y2=y+size;
    XDrawSegments(display,window,gc,s,2);
    break;
   case '*':
    ...
    XDrawSegments(display,window,gc,s,3);
    break;
   case 'x':
    dx=size; dy=size;
    ...
    break;
   case 'b':
    XDrawRectangle(display,window,gc,x-size,y-size,2*size,2*size);
    break;
   case 'B':
    XFillRectangle(display,window,gc,x-size,y-size,2*size+1,2*size+1);
    break;
   case 'd':
   ...
   case 'D':
    ...
    p[3].x=x;		p[3].y=y-size;
    p[4].x=x-size;	p[4].y=y;
    XFillPolygon(display,window,gc,p,5,Convex,CoordModeOrigin);
    break;
  }
}
...
void dvend(void)
{
 switch (polymode)
 {
  ...
  case 'l':				/* open polygonal line */
   XDrawLines(display,window,gc,point,points,CoordModeOrigin);
   break;
  case 'f':				/* filled polygon */
   dvpoint(point[0].x,point[0].y);
   XFillPolygon(display,window,gc,point,points,Complex,CoordModeOrigin);
   break;
  case 'm':				/* polymarker (only dots) */
   XDrawPoints(display,window,gc,point,points,CoordModeOrigin);
   break;
 }
 polymode=0;
 points=0;
