/*
* aa.c
* naive implementation of affine arithmetic
...
/* #define aa_newsymbol()	(++last) */

real aa_norm(AAform a)				/* \sum_1^n a_i */
{
 int i;
 real na=0.0;
 forallnoises(i) na+=fabs(a[i]);
 return a[NORM]=na;
}

#undef aa_norm
#define aa_norm(a)	((a[NORM]>=0.0)? a[NORM] : (aa_norm)(a))

void aa_set(AAform a, AAform b)			/* a = b */
{
 int i;
 forallsymbols(i) a[i]=b[i];
 a[NORM]=b[NORM];
}
...
void aa_mul(AAform a, AAform b, AAform c)	/* a = b*c */
{
 ...
 real nb=aa_norm(b);
 real nc=aa_norm(c);
 forallnoises(i) a[i]=b[0]*c[i]+b[i]*c[0];
 i=aa_newsymbol();
 a[i]=nb*nc;
 a[0]=b[0]*c[0];
 a[NORM]=NONORM;
...
{
 int i;
 real nb=aa_norm(b); nb=(nb*nb)/2.0;
 forallnoises(i) a[i]=2.0*b[0]*b[i];
 i=aa_newsymbol();
 a[i]=nb;
 a[0]=b[0]*b[0]+nb;
#if 1
...
void aa_condense(AAform a, int from, int at)
{
 int i;
 real na=0.0;
 for (i=from; i<=last; i++) { na+=fabs(a[i]); a[i]=0.0; }
 a[at]=na;
 a[NORM]=NONORM;
}
...
void aa_stats(void)
{
 fprintf(stderr,"aa: %d symbols used out of %d\n",last,N-1);
}
...
void aa_trace(char* name, AAform a)
{
 ...
 fprintf(stderr,"aa: %p %s= %g .. %g\t|%g|\t",a,name,amin,amax,a[NORM]);
 fprintf(stderr,"%g",a[0]);
 forallnoises(i) if (a[i]!=0) fprintf(stderr," %+g@%d",a[i],i);
 fprintf(stderr,"\n");
}
