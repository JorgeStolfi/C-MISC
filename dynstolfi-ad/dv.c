/*
* Last edited on 2016-09-13 15:39:38 by stolfilocal
* dv/X11.c
...
void dvclip(int xmin, int xmax, int ymin, int ymax)
{
 ...
 r.x=(short int)xmin;
 r.y=(short int)ymin;
 r.width=(short int)(xmax-xmin+1);
 r.height=(short int)(ymax-ymin+1);
...
{
 XDrawRectangle(display,canvas,gc,xmin,ymin,xmax-xmin,ymax-ymin);
}
 ...
 for (m=marktype; *m!=0; m++)
  switch (*m)
  {
   ...
   case 'O':
    ...
    break;
   case '+':
    s[0].x1=(short int)(x-size); s[0].y1=(short int)y;        s[0].x2=(short int)(x+size); s[0].y2=(short int)y;
    s[1].x1=(short int)x;        s[1].y1=(short int)(y-size); s[1].x2=(short int)x;        s[1].y2=(short int)(y+size);
    XDrawSegments(display,canvas,gc,s,2);
    break;
   case '*':
    dx=(int)(size*0.5); dy=(int)(size*0.866);
    s[0].x1=(short int)(x-size);     s[0].y1=(short int)y;        s[0].x2=(short int)(x+size); s[0].y2=(short int)y;
    s[1].x1=(short int)(x+dx);       s[1].y1=(short int)(y+dy);   s[1].x2=(short int)(x-dx);   s[1].y2=(short int)(y-dy);
    s[2].x1=(short int)(x+dx);       s[2].y1=(short int)(y-dy);   s[2].x2=(short int)(x-dx);   s[2].y2=(short int)(y+dy);
    XDrawSegments(display,canvas,gc,s,3);
    break;
   case 'x':
    dx=(int)size; dy=(int)size;
    s[0].x1=(short int)(x+dx);       s[0].y1=(short int)(y+dy);   s[0].x2=(short int)(x-dx);   s[0].y2=(short int)(y-dy);
    s[1].x1=(short int)(x+dx);       s[1].y1=(short int)(y-dy);   s[1].x2=(short int)(x-dx);   s[1].y2=(short int)(y+dy);
    XDrawSegments(display,canvas,gc,s,2);
    break;
   case 'b':
   ...
   case 'B':
    ...
    break;
   case 'd':
    p[0].x=(short int)(x-size);      p[0].y=(short int)y;
    p[1].x=(short int)x;             p[1].y=(short int)(y+size);
    p[2].x=(short int)(x+size);      p[2].y=(short int)y;
    p[3].x=(short int)x;             p[3].y=(short int)(y-size);
    p[4].x=(short int)(x-size);      p[4].y=(short int)y;
...
int dvpoint(int x, int y)               /* should check XMaxRequestSize */
{
 ...
 point[i].x=(short int)x;
 point[i].y=(short int)y;
 return points;
}
...
void dvdoublebuffer(int on)
{
 if (on)
 {
  back=XCreatePixmap(display,window,(unsigned int)dv.xu,(unsigned int)dv.yu,8);
  dvbackbuffer();
  dvclear(0);
 }
 else if (back!=0)
...
{
 XCopyArea(display,back,window,gc,0,0,(unsigned int)dv.xu,(unsigned int)dv.yu,0,0);
#if 0
 XFlush(display);
#endif
}
...
void dvdump(int n)
{
 char b[80];
 dvflush();
 sprintf(b,"xwd -nobdrs -id %d | gzip -c >out%04d.xwd.gz",(int)window,n);
 system(b);
}
