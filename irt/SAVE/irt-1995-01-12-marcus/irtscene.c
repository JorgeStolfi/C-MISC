...
void irt_read_parm_file (char *scene_name, scene_t *sc);
  /*
    Reads the viewing, shading, and imaging parameters from
    the <scene_name>.parms file. */

void irt_read_pcode_file (char *proc_name, pcode_proc_t *proc);
  /* 
...
void irt_allocate_regs_and_stack(shape_t *sh);
  /* 
    Allocates the stack and registers for the evaluation of sh->proc,
    as specified by sh->arithmetic. */
...
void irt_read_scene (char *scene_name, scene_t *sc)
  { int i;
    sc->name = scene_name;
    irt_read_parm_file(scene_name, sc);
    ...
    irt_read_pcode_file(sc->shape.proc_name, &(sc->shape.proc));
...
void irt_set_parm_defaults(scene_t *sc)
  { 
    ...
    sc->shape.arithmetic = arith_aa;
    
    /* Looks: */
    sc->looks.ambient_color    = (rgb_t) {{0.05, 0.05, 0.05}};
    sc->looks.matte_color      = (rgb_t) {{0.05, 0.60, 0.05}};
    sc->looks.shiny_color      = (rgb_t) {{0.20, 0.20, 0.10}};
    sc->looks.shiny_spread     = 0.9;
    sc->looks.max_bounces      = 3;
    sc->looks.background_color = (rgb_t) {{0.10, 0.10, 0.60}};
    ...
    { /* Default light number 0 */
      ...
      sc->light[0].color   = (rgb_t) {{0.8, 0.8, 0.8}};
...
void irt_read_parm_file (char *scene_name, scene_t *sc)
  {
    FILE *parm_file = open_read(txtcat(scene_name, ".parms"), TRUE);
    char *s; /* Current line */
    int nlines = 0;
    ...
    while (s != NULL)
      {
        ...
        if (t == NULL) 
          ...
          { char *a = strtok(NULL, " ");
            if (strcmp(a, "IA") == 0) 
              { sc->shape.arithmetic = arith_ia; }
            else if (strcmp(a, "IA/DIFF") == 0) 
              { sc->shape.arithmetic = arith_ia_diff; }
            else if (strcmp(a, "AA") == 0)
              { sc->shape.arithmetic = arith_aa; }
            else
              { error ("irt_read_parm_file: unknown arithmetic"); }
          }
          ...
          { error ("irt_read_parm_file: unrecognized keyword"); }
    ...
    if (sc->shape.proc_name == NULL) 
      { error ("irt_read_parm_file: missing parameter proc_name"); }
    fclose(parm_file);
    fprintf(stderr, "----------------------------------------------\n");
  }
    ...
    while (s != NULL)
      { char *t;
        ...
        if (strcmp(t, "position") == 0)
          ...
          { error ("irt_read_light_parms: unrecognized keyword"); }

        /* Check for bogus input: */
        affirm (strtok(NULL, " ") == NULL, "irt_read_parm_file: superfluous parameters");
        if (done) 
          { if (lt->position.c[0] == Infinity)
	      { error("irt_read_light_parms: missing light position"); }
            return;
          }
        s = read_line(parm_file);
      }
    error("irt_read_light_parms: missing end_light"); 
  }
...
void irt_compute_screen_axes(view_t *vw)
  {
    ...
    (void) r3_normalize(&sx);
    
    r3_cross(&sz, &sx, &sy);
    (void) r3_normalize(&sy);
...
void irt_compute_looks_flags(looks_t *lk)
  { lk->has_shiny = rgb_is_black(&(lk->shiny_color));
...
void irt_read_pcode_file(char *proc_name, pcode_proc_t *proc)
  { FILE *pcode_file = open_read(txtcat(proc_name, ".pcode"), TRUE);
